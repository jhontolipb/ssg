...
-- Allow anyone to view all users (simplify for now)
...
CREATE POLICY "Organization admins can update events" ON events
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM organization_members 
      WHERE user_id = auth.uid() 
...
CREATE POLICY "Anyone can insert clearances" ON clearances
  FOR INSERT WITH CHECK (true);
